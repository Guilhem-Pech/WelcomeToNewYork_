...
void LateUpdate ()
    ...
    float pixelPerfectZOffset = halfPixelOffsetInWorldUnits - Mathf.Repeat (snapSizeWorldUnits + Mathf.Repeat (camera.transform.position.z, snapSizeWorldUnits), snapSizeWorldUnits);
    float pixelPerfectZYOffset = (pixelPerfectYOffset + pixelPerfectZOffset) / 2;
    ...
    camera.projectionMatrix = Matrix4x4.Ortho (
            -halfWidth + pixelPerfectXOffset,
             halfWidth + pixelPerfectXOffset,
            -halfHeight + pixelPerfectZYOffset,
             halfHeight + pixelPerfectZYOffset,
